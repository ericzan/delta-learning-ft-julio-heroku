...
export class ViewComponent implements OnInit {
  ...
  pkgId: string = '';
  constructor(private watchService: WatchServiceVw, private uiOperGrService: UiOperGrService, private wathSteps: WathStepsLearningService, private router: Router, private route: ActivatedRoute, private storage: LocalStorageService ) {
  }
  setRandomResponse() {
    this.listResponse = this.listResponse.sort(() => Math.random() - 0.5);
  }
  ngOnInit(): void {
    this.wathSteps.setCurrentStep(3);
    this.wathSteps.onShuffle().subscribe(resp => {
      this.setRandomResponse();
    });
    this.route.parent?.parent?.paramMap.pipe(take(1)).subscribe(resp => {
      console.log('params vw', String(resp.get('id')));
  ...
  validateSelectedRequestAndResponse() {
    if (this._isSelectedResponse && this._isSelectedRequest && this._isSelectedResponse.id !== undefined && this._isSelectedRequest.id !== undefined) {
      ...
      this.wathSteps.setWatchCardSelectedIsValid(this._isSelectedRequest.id, isValid);
      this.wathSteps.setWatchCardSelectedIsValid(this._isSelectedResponse.id, isValid);
      this.countClicks++;
      if (isValid === "success") {
        this.count_success++;
      } else {
  ...
  clickNext(){
    this.wathSteps.setNextStep(4);
    this.router.navigate(['../', 'verifying-ce'], {
  ...
  clickSend() {
    ...
    const level = 'lvl_30_01';
    const clicksQty = this.count_success + this.count_errors;
